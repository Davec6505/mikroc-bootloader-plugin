...
function generateMikroCMakefileContent(
    ...
    projectName: string,
    _sourceFiles: string[],  // unused — wildcard discovers them at build time
    emclLibs: string[],      // user-selected libs e.g. ['__Lib_Delays.emcl']
    _makePath: string,
    _binPath: string,
    _shPath: string,
    projectPath: string      // absolute output project path
): string {
    const mikroCDevice = 'P' + config.device;
    const clockMHz     = Math.floor(Math.round(config.clock.systemFrequency) / 1_000_000);
    const heapSize     = config.build?.heapSize ?? 4096;

    // Windows backslash paths for MikroC
    const installWin  = compilerInstallPath.replace(/\//g, '\\');
    ...
    const cflags = `-MSF -DBG -p$(DEVICE) -HEAP ${heapSize} -Y -DL -SSA -EBASE 0x9FC01000 -INTDEF MV_SRS7_IS32 -O11111113 -fo$(CLOCK)`;
...
# Compiler and paths (override: make COMPILER="your\\path\\mikroCPIC32.exe")
COMPILER := ${installWin}\\mikroCPIC32.exe
